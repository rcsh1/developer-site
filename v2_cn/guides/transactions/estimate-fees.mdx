...
|-----------|--------------------|---------------------------------|-------------------------|
| Legacy 模型    | 交易费用 = Gas Price * Used Gas Units<ul><li>Gas Price：以 wei 为单位的 Gas 价格，代表每使用一个 Gas Unit 处理交易需支付给验证者的 ETH 数量。</li><li>Gas Unit：在以太坊网络上执行操作所需的计算工作量的度量。</li></ul>   |      以太坊和其他 EVM 兼容链      |           <ul><li>Gas Price</li><li>Gas Limit</li></ul>请参考 [估算交易费用](/v2/api-references/transactions/estimate-transaction-fee) 操作的响应获取详细信息。          | 
| [EIP-1559](https://eips.ethereum.org/EIPS/eip-1559)  | 交易费用 = (Base Fee Per Gas + Priority Fee Per Gas) * Used Gas Units<ul><li>Base Fee Per Gas：每使用一个 Gas Unit 在链上执行交易所需的基础费用，根据网络需求动态调整。</li><li>Priority Fee Per Gas：每使用一个 Gas Unit 向矿工支付的额外费用，以优先处理交易。</li><li>Gas Unit：在以太坊网络上执行操作所需的计算工作量的度量。</li></ul>  | 当前支持的链为 Ethereum Mainnet、Sepolia Testnet、Scroll Mainnet、Base、BNB Smart Chain、Polygon、Sonic 和 Berachain。 | <ul><li>Max Gas Price Per Gas</li><li>Max Priority Fee Per Gas</li><li>Gas Limit</li></ul>请参考[估算交易费用](/v2/api-rereferences/transactions/estimate-transaction-fee) 操作的响应获取详细信息。  | 
| UTXO 模型      | 交易费用 = Fee Rate * Transaction Size<ul><li>Fee Rate：以 sat/vByte 为单位的费率，代表每字节数据在区块链上占用的空间所需支付的 satoshis 数量。</li><li>Transaction Size：交易在区块链上占用的空间大小，影响 UTXO 模型中的总费用。</li></ul> | 比特币和其他基于 UTXO 模型的链  | <ul><li>Max Fee Amount</li><li>Fee Rate</li></ul>请参考 [估算交易费用](/v2/api-references/transactions/estimate-transaction-fee) 操作的响应获取详细信息。             |
| 固定模型     | 交易费用为固定金额。  | TRON                            | Max Fee Amount。参考 [估算交易费用](/v2/api-references/transactions/estimate-transaction-fee) 操作的响应获取详细信息。                   |
| [Solana](https://solana.com/docs/core/fees) | 交易费用 = Base Fee + Compute Unit Limit * Compute Unit Price<ul><li>Base Fee：每个签名固定收取的费用（默认每个签名 5,000 Lamports）。</li><li>Compute Unit Limit：交易可消耗的最大计算单元数量。</li><li>Compute Unit Price：每个计算单元的单价，单位为 Lamports，用于提升交易的优先级。</li><li>Rent（如适用）：适用于未达到租金豁免条件的账户，网络会定期扣除该费用以补偿链上存储资源。该费用不属于交易手续费的一部分，但当账户余额低于豁免门槛时可能产生。</li></ul> | Solana | <ul><li>Compute Unit Limit</li><li>Compute Unit Price</li></ul>请参考 [估算交易费用](/v2/api-references/transactions/estimate-transaction-fee) 操作的响应获取详细信息。 |
| [Filecoin](https://spec.filecoin.io/systems/filecoin_vm/gas_fee/#section-systems.filecoin_vm.gas_fee.summary) | 交易费用 = min(Gas Fee Cap, Base Fee + Gas Premium) * Gas Limit<Note>链上实际费用计算方式为：<code>Base Fee * Gas Used + Gas Premium * Gas Limit</code>。</Note><ul><li>Gas Fee Cap：你愿意为每单位 Gas 支付的最高价格。</li><li>Base Fee：交易打包所需的最低费用，随网络拥堵动态调整，并在支付后销毁，从而减少 FIL 总供应量。</li><li>Gas Premium：用户指定的矿工小费，用于提升交易优先级。</li><li>Gas Limit：交易可消耗的最大 Gas 数量。</li></ul> | Filecoin | <ul><li>Gas Limit</li><li>Gas Premium</li><li>Gas Fee Cap</li></ul>请参考 [估算交易费用](/v2/api-references/transactions/estimate-transaction-fee) 操作的响应获取详细信息。 |
