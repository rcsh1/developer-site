...
- API Key 包含在 API 请求中，使 WaaS 服务能够识别客户端。使用前您需要在 Cobo Portal 上注册 API Key。更多信息，请参阅[注册 API Key](https://manuals.cobo.com/cn/portal/developer-console/api-keys)。
...
生成 API Key 后，您需要在 Cobo Portal 上注册密钥并配置相关权限。要了解如何注册 API Key，请参阅[注册 API Key](https://manuals.cobo.com/cn/portal/developer-console/api-keys)。
