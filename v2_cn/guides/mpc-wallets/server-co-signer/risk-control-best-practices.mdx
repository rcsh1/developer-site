...
2. 验证关键的交易参数以防止未经授权或恶意的交易：
   - 根据特定链的格式解析 `extra_info.Transaction.RawTxInfo.UnsignedRawTx`，提取以下关键参数：
     - Gas 费用：确保在可接受的范围内
     - 目标地址：验证是否在允许的地址列表中
     - 交易金额：检查是否符合配置的限制
     - 链 ID：确认是否匹配您预期的网络
   ...
   - 将解析出的参数与 `extra_info.Transaction` 中的值进行比对
   - 将解析出的参数或 `extra_info.Transaction` 与通过 WaaS 2.0 API 构建的原始交易进行比对
