...
- 调用 [Create order link](/v2/api-references/payment/create-order-link) 创建一个订单支付链接。该链接会跳转到由 Cobo 提供的支付页面，用户可在该页面创建订单并完成支付，无需您自行开发前端交互流程。您还可以通过 iFrame 方式将该支付页面嵌入到您的网站或应用中。
...
您可以调用 [Create pay-in order](/v2/api-references/payment/create-pay-in-order) 来创建一个支付订单。创建订单时，您需要指定商品订单金额、用户支付所用的加密货币等信息，系统会根据这些信息计算出订单的应付金额（`payable_amount`）。应付金额的计算公式为：

**应付金额** = **（商品订单金额 + 开发者费用）** / **汇率**

- **应付金额**：用户实际需支付的加密货币金额，以 `token_id` 指定的加密货币为单位。例如，若 `token_id` 为 `ETH_USDT`，则用户需通过 ETH 链支付相应金额的 USDT。
...
- **开发者费用**：您在创建订单时传入的 `fee_amount` 字段。如果您是服务多个下游商户的平台机构，需要在您和下游商户之间分配收入，可以通过设置该费用来实现。开发者费用和商品订单金额之间的比例决定了您和下游商户之间的收入分成比例。更多信息请参考[资金分配与余额管理](/v2_cn/payments/amounts-and-balances)。
  <Info>如果您是商户（直接服务于用户），通常无需设置开发者费用。</Info>
- **汇率**：
...
| `payable_amount`（用户最终应付的金额） | `"101.01"` | `"100"` | `"104.08"` | `"102"` |
