...
您可以通过 Payment API 或 Payment Management App 对充值模式下的商户余额或开发者余额进行结算。您可以参考[金额定义与余额计算说明](/v2_cn/payments/amounts-and-balances)获取更多关于商户余额和开发者余额的说明。
...
* **商户余额结算**（`Merchant`）：从指定商户的余额中扣除结算金额。您可以在一个结算请求中结算多个商户的余额。
...
调用 [Get withdrawable balances](/v2/api-references/payment/get-withdrawable-balances) 查询指定商户余额或开发者余额，确定您可以结算的最大金额。充值模式下，您需要将`acquiring_type` 字段设置为 `TopUp`。

API 返回以下两个关键余额字段：

- `available_balance`：可结算余额，即已达到[归集阈值](/v2_cn/payments/glossary#sweep-threshold（归集阈值）)且可立即用于结算的资金
- `total_balance`：总余额，包括可结算余额和未达到归集阈值的资金

当 `available_balance` 小于 `total_balance` 时，说明部分资金尚未达到归集阈值。您有两种处理方式：
...
调用 [Create settlement request](/v2/api-references/payment/create-settlement-request) 创建结算请求。请求中的关键参数包括：

#### 基础参数
* `request_id`：您提供的唯一请求 ID，用于跟踪结算请求
* `acquiring_type`：收单模式。充值模式下，您需要将该字段设置为 `TopUp`
* `settlement_type`：选择资金来源
  - `Merchant`：从商户余额中扣除结算金额。
...
| 资金来源 | 结算方式 | 必需参数 |
|---------|---------|---------|
| **商户余额** | 加密货币转账 | `merchant_id`、`amount`、`token_id`、`crypto_address_id` |
| **商户余额** | 法币出金 | `merchant_id`、`amount`、`currency`、`bank_account_id` |
