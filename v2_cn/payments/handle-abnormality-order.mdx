...
* 对于全托管钱包或者启用了 Cobo KYC 的 MPC 钱包，Cobo 将对每笔转入收币地址的交易进行合规扫描（如 KYT/AML 扫描）。
* 对于未启用 Cobo KYC 的 MPC 钱包，您可通过 [Screening App](https://manuals.cobo.com/cn/apps/screening/introduction) 自行对资金进行合规扫描，并处理未通过的交易。


当某笔交易初次未通过合规扫描时，系统会触发 `payment.transaction.held` 事件，后续处理流程如下：
...
- 若该笔交易最终未通过人工审核：
    - 资金将被隔离或冻结，不会被计入订单实收金额
    - 订单状态将保持不变
    - 您需要通过 [help@cobo.com](mailto:help@cobo.com) 联系 Cobo 的支持团队处理被隔离/冻结的资金
