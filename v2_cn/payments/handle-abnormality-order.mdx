...
| **少付** | 在订单有效期内，用户实付金额少于应付金额 | 订单状态为 `Underpaid`（终态）。|
| **晚付** | 用户在订单过期后进行首次或再次付款 | 不会改变订单状态。每次晚付都会触发一次 `payment.transaction.late` Webhook 事件。|

[金额定义与余额计算说明](/v2_cn/payments/amounts-and-balances)中详细介绍了多付、少付和晚付情况下，Cobo 对资金的处理方式。对于异常支付带来的多余资金，您可以根据业务需求选择以下处理方式：
