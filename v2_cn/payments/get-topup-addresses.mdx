...
您可以调用 Payment API 为付款方获取专用的充值地址。下图展示了付款方、商户、开发者以及 Cobo 之间的交互流程：
...
sequenceDiagram
    ...
    付款方 ->> 商户: 请求开通钱包/充值功能
    商户 ->> 开发者: 请求开通钱包
    开发者 ->> Cobo: ① 获取该付款方的充值地址
    Cobo ->> 开发者: 返回充值地址
    Cobo ->> Cobo: 地址入账轮询
    开发者 ->> 商户: 返回充值地址
    商户 ->> 付款方: 显示充值地址
    付款方 ->> Cobo: 向地址转入资金进行充值
    Cobo ->> Cobo: 合规检查
...
1. 调用 [Get top-up address](/v2/api-references/payment/get-top-up-address) 为指定商户的指定付款方创建充值地址。请求中的关键参数包括：
...
2. **配置充值事件监听**：通过 Webhook 接收充值相关的事件通知，包括：
   ...
   - `payment.transaction.held`：充币交易正在接受合规扫描
