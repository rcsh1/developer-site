...
* 如果您选择使用 MPC 钱包，请参考 [MPC 钱包快速入门](https://manuals.cobo.com/cn/portal/quick-start-guide-mpc)，完成以下步骤：  
  1. 设置账户和团队。  
  2. 配置一个 MPC Vault。由于结算之前可能会发生大量归集交易且交易时间集中，请确保您已设置至少一个[服务端签名人](/v2_cn/guides/mpc-wallets/server-co-signer/tss-node-overview)作为您的[主控组](https://manuals.cobo.com/cn/portal/mpc-wallets/ocw/holder-group-main-group)或[签名组](https://manuals.cobo.com/cn/portal/mpc-wallets/ocw/holder-group-signing-group)，以实现自动化签名。  
...
1. 参考[开发者指南](/v2_cn/guides/get-started/get-started-with-waas)，生成一对 API Key 和 API Secret，并在 Cobo Portal 注册该 API Key：  
   * **用户角色**：根据业务需求，选择 **Payment Admin** 或 **Payment Member**。  
   * **钱包范围**：根据您的业务需求和安全策略选择合适的钱包范围：
     - 测试阶段：可以选择**任何类型钱包**以便快速验证集成功能
...
在您日常开展业务过程中，需要通过 [Fee Station](https://manuals.cobo.com/cn/portal/fee-station/introduction) 管理在 Cobo 产生的各类费用。请联系 Cobo 销售团队开通该功能。
