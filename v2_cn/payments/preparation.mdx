...
如果您需要调用 Payment API，则需要参考以下步骤，完成 API Key 的配置。
...
3. 填写钱包名称，并勾选**自动归集**，以确保后续结算和退款操作的顺利进行。
...
您可以使用 Payment Management App 或 Payment API 创建商户并与钱包关联。当您为该商户创建订单或者充值地址时，Cobo 均会在该钱包下创建相应的收币地址。**每个商户需对应唯一钱包，同一个钱包可以关联一个或多个商户。**

<Note>如果您是商户（直接服务付款方），则仅需创建一个商户（商户名称可以是任意名称），且无需配置开发者费率。开发者费率仅适用于商户平台（如 PSP、电商平台）向其下游商户收取手续费。</Note>
...
除 Payment Management App 外，您也可以使用 Payment API 创建商户。
...
根据您的资金结算需求，请调用 Payment API 或联系 Cobo 销售团队录入相应的结算账户。

| 结算账户类型 | 说明 |
| --- | --- | 
| **加密货币地址** | 调用 [Create crypto address](/v2/api-references/payment/create-crypto-address) 或联系 Cobo 销售团队录入外部加密货币地址。资金将直接转账至该钱包地址。|
| **银行账户** | 联系 Cobo 销售团队录入银行账户信息。资金将被兑换为法币并结算至账户。注意：开发环境下，不支持实际的法币兑换和结算。您可以调用 [List all bank accounts](/v2/api-references/payment/list-all-bank-accounts) 查询已录入的银行账户信息。 | 
...
根据您选择的收单模式，您可以参考以下文档完成后续集成步骤：

| 收单模式 | 文档链接 |
| --- | --- |
| 订单模式 | • [使用 App 快速跑通订单模式](/v2_cn/payments/order-mode-quick-start)<br/><br/>• 使用 API 集成：<br/>&nbsp;&nbsp;&nbsp;&nbsp;- [创建订单](/v2_cn/payments/create-orders)：了解如何创建支付订单<br/>&nbsp;&nbsp;&nbsp;&nbsp;- [结算](/v2_cn/payments/settle-orders)：了解如何结算商户余额和开发者余额<br/>&nbsp;&nbsp;&nbsp;&nbsp;- [异常处理](/v2_cn/payments/handle-abnormality-order)：了解如何处理多付、少付、退款等异常情况 |
| 充值模式 | • [使用 App 快速跑通充值模式](/v2_cn/payments/top-up-mode-quick-start)<br/><br/>• 使用 API 集成：<br/>&nbsp;&nbsp;&nbsp;&nbsp;- [获取充值地址](/v2_cn/payments/get-topup-addresses)：了解如何创建充值地址<br/>&nbsp;&nbsp;&nbsp;&nbsp;- [结算](/v2_cn/payments/settle-topup)：了解如何结算充值模式下的资金<br/>&nbsp;&nbsp;&nbsp;&nbsp;- [异常处理](/v2_cn/payments/handle-abnormality-topup)：了解如何处理地址更换、合规异常等情况 |
