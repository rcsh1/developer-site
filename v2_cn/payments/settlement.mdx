...
with ApiClient(configuration) as api_client:
   ...
   settlement = CreateSettlement(
       merchant_id="<YOUR_MERCHANT_ID>",  # 替换为商户 ID
       token_id="TRON_USDT",              # 结算使用的币种 ID
       currency="USD",                    # 目标法币
       amount="10",                       # 结算金额（以目标法币为单位）
       bank_account_id="<YOUR_BANK_ACCOUNT_ID>",  # 收款银行账户 ID
       settlement_type=SettlementType.MERCHANT  # 结算类型
   )
