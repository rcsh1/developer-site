...
以下为调用"Create refund link"接口的示例代码：

<Accordion title="Python示例代码">
```py
import cobo_waas2
...
with cobo_waas2.ApiClient(configuration) as api_client:
   ...
   try:
       ...
       print("退款地址收集URL:", api_response.url)
       print("退款请求标识:", api_response.token)
   except Exception as e:
       print("Exception when calling PaymentApi->create_refund_link: %s\n" % e)
```
</Accordion>

<Accordion title="Java示例代码">
```java
...
public class Example {
 public static void main(String[] args) {
   ...
   try {
          ...
          System.out.println("退款地址收集URL: " + refundLink.getUrl());
          System.out.println("退款请求标识: " + refundLink.getToken());
...
<img src="/v2_cn/images/payments/refund-address-collection-default.png" className="screenshot_full_screen" alt="退款地址收集页面-默认地址" />

**更改退款地址页面**（需要填写新地址和Memo）：

<img src="/v2_cn/images/payments/refund-address-collection-change.png" className="screenshot_full_screen" alt="退款地址收集页面-更改地址" />
...
try:
   ...
   print("退款状态:", refund_detail.status)
   print("退款金额:", refund_detail.amount)
   print("退款地址:", refund_detail.to_address)
except Exception as e:
...
| 错误码 | 说明 | 处理建议 |
|--------|------|----------|
| `Insufficient_balance` | 可退金额不足 | 检查账户余额，确保有足够的资金用于退款 |
| `refund_in_progress` | 已存在进行中的退款 | 已进入自动退款流程的订单请勿在后台再次"手动退款"，避免重复出款 |
