...
调用 [Get withdrawable balances](/v2/api-references/payment/get-withdrawable-balances) 查询指定商户余额或开发者余额，确定您可以结算的最大金额。订单模式下，您需要将`acquiring_type` 字段设置为 `Order`。
...
- 等待该地址上的余额增加至归集阈值。
- 调用 [Create forced sweep](/v2/api-references/payment/create-forced-sweep) 进行强制归集：Cobo 会立即将未达到归集阈值的资金归集到归集地址。归集完成后，您可以将这部分资金用于结算。
