...
Use either OpenSSL or the Python library to generate an Ed25519 key pair as the API key and API secret. This guide uses OpenSSL as an example. To learn more about generating an API key and an API secret, see [Generate an API key and an API secret](/v2/guides/overview/cobo-auth#generate-an-api-key-and-an-api-secret).
