...
This guide gets you started with using [Custodial Wallets (Asset Wallets)](https://manuals.cobo.com/en/portal/custodial-wallets/introduction). By following this guide, you will be able to use the WaaS 2.0 API to:

1. Query the chains and tokens you can use
2. Create an Asset Wallet
3. Generate a deposit address within the wallet
4. Deposit tokens into the wallet and track the transaction status
5. Withdraw tokens from the wallet
6. Query wallet balances
...
- Follow the instructions in [Get started with WaaS 2.0](/v2/guides/get-started-with-waas) to set up your account and send your first API request to the WaaS 2.0 service.
...
To retrieve the chains you can use for Asset Wallets and their corresponding chain IDs, call [List enabled chains](/v2/api-references/wallets/list-enabled-chains) and specify the query parameters as follows:
...
public class ListEnabledChainsExample {
    public static void main(String[] args) {
        ...
        try {
            ...
            String after = null;
            // Query enabled chains
            ListSupportedChains200Response result = apiInstance.listEnabledChains(WalletType.CUSTODIAL, WalletSubtype.ASSET, limit, before, after);
...
If you want to use a chain that is not already on the list, you need to enable the chain first. To enable a new blockchain, follow the steps below:

- Log into [Cobo Portal](https://portal.dev.cobo.com/).
- Click <img src="/v2/images/guides/wallets-icon.svg" className="icon" alt="Wallets icon"></img> and select **Custodial Wallets**.
- Click <img src="/v2/images/guides/token_management.svg" className="icon" alt="Token management icon"></img> on the top-right corner of the page, and then follow the instructions on the screen to enable the blockchain of your choice. 
...
To retrieve the tokens you can use for Asset Wallets and their corresponding token IDs, call [List enabled tokens](/v2/api-references/wallets/list-enabled-tokens) and specify the query parameters as follows:

- `wallet_type`: `Custodial`.
- `wallet_subtype`: `Asset`.
- `chain_ids`: Specify the chain of your choice. You can also leave this parameter empty to query all chains.
...
public class ListEnabledTokensExample {
    public static void main(String[] args) {
        ...
        try {
            ...
            String after = null;
            // Query enabled tokens
            ListSupportedTokens200Response result = apiInstance.listEnabledTokens(WalletType.CUSTODIAL, WalletSubtype.ASSET, chainIds, limit, before, after);
            System.out.println(result);
        } catch (ApiException e) {
...
Now that you have confirmed which chains and tokens you can use, the next step is to set up a wallet. To create an Asset Wallet, call the [Create wallet](/v2/api-references/wallets/create-wallet) operation and specify the properties in the request body as follows:
...
Webhook is an essential mechanism for the WaaS service to communicate with your application. After you register a callback endpoint on Cobo Portal, the WaaS service sends push messages to the designated URL when an event occurs.
...
To learn the trigger condition and data structure of each event type, refer to [Webhook event type and event data](/v2/guides/webhooks-callbacks/webhook-event-type).
...
Now that you have tokens in your wallet, it's time to try withdrawing them. To withdraw tokens from your wallet, you must first set up a callback endpoint and register it on Cobo Portal. Once you initiate a withdrawal using the WaaS 2.0 API, the callback endpoint will receive a callback message containing the transaction details. The transaction will proceed only if you approve the withdrawal request.
