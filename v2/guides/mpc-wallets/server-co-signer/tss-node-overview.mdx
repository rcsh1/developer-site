...
A task request must be approved by both the embedded risk control module and all configured TSS Node callback servers to proceed. If any component (embedded or callback) rejects the operation, it will be denied.
