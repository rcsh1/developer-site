...
| Field | Type | Description |
|-------|------|-------------|
| group_id | `string` | The ID of the key share holder group. |
...
| staking_activity | `object` | The information of the staking activity, if the transaction is a staking activity. For details, see the response of the [Get staking activity details](/v2/api-references/stakings/get-staking-activity-details) operation.  |
...
| Field | Type | Description |
|-------|------|-------------|
| old_group_id | `string` | The ID of the existing key share holder group. |
...
| request_id | `string` | The unique ID of the callback request.|
| action | `string` | Action to take. Possible values:<br/>- `ACCEPT`: Approve the request and proceed with the operation<br/>- `REJECT`: Deny the request and stop the operation |
...
- For `TypePing` requests, a successful response (status = `0`) is required to indicate server availability.