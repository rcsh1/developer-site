...
2. Validate critical transaction parameters to prevent unauthorized or malicious transactions
    ...
    - Parse `extra_info.Transaction.RawTxInfo.UnsignedRawTx` according to the chain format, and extract key transaction parameters such as:
    ...
    - Compare the parsed parameters with those in `extra_info.Transaction`
    - Compare the parsed parameters or `extra_info.Transaction` with the original transaction created using the WaaS 2.0 API
