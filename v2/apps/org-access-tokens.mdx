...
5. Your app acquires an Org Access Token from the WaaS service by calling the [Get Org Access Token](/v2/api-references/oauth/get-access-token) operation. 
6. Your app accesses the WaaS 2.0 service with the Org Access Token. 
7. When the Org Access Token expires, your app calls the [Refresh Org Access Token](/v2/api-references/oauth/refresh-access-token) operation to get a new Org Access Token with the Refresh Token.
...
Your app will need to call [Get access token](/v2/api-references/oauth/get-access-token) to get Org Access Tokens and call [Refresh access token ](/v2/api-references/oauth/refresh-access-token)to refresh Org Access Tokens when they expire. When calling these two operations, your app must provide the app key, client ID, and organization ID as inputs.
