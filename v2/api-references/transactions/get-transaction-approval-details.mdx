---
openapi: get /transactions/{transaction_id}/approval_detail
---
...
func main() {
	...
	resp, r, err := apiClient.TransactionsAPI.GetTransactionApprovalDetail(ctx, transactionId).
		Execute()
	if err != nil {
		fmt.Fprintf(
			os.Stderr,
			"Error when calling `TransactionsAPI.GetTransactionApprovalDetail``: %v\n",
			err,
		)
	...
	fmt.Fprintf(
		os.Stdout,
		"Response from `TransactionsAPI.GetTransactionApprovalDetail`: %v\n",
		resp,
	)
}
...
apiInstance.getTransactionApprovalDetail(transaction_id).then(
  (data) => {
