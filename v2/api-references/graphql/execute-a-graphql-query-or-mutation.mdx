...
public class Example {
  public static void main(String[] args) {
    ...
    GraphQLRequest graphQLRequest =
        new GraphQLRequest().query("query { wallets { edges { node { id name } } } }");
    try {
...
func main() {
	...
	resp, r, err := apiClient.GraphQLAPI.ExecuteGraphql(ctx).
		GraphQLRequest(graphQLRequest).
		Execute()
	if err != nil {
...
const opts = {
  GraphQLRequest: CoboWaas2.GraphQLRequest.constructFromObject({
    query: "query { wallets { edges { node { id name } } } }",
  }),
};
apiInstance.executeGraphql(opts).then(
  (data) => {
