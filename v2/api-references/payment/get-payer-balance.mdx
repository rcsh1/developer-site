---
openapi: get /payments/balance/payer/address
---
...
with cobo_waas2.ApiClient(configuration) as api_client:
    ...
    try:
        # Get payer balance
        api_response = api_instance.get_payer_balance_by_address(
            payer_id, token_id, merchant_id=merchant_id
        )
        print("The response of PaymentApi->get_payer_balance_by_address:\n")
        pprint(api_response)
    except Exception as e:
        print(
            "Exception when calling PaymentApi->get_payer_balance_by_address: %s\n" % e
        )
...
public class Example {
  public static void main(String[] args) {
    ...
    try {
      List<ReceivedAmountPerAddress> result =
          apiInstance.getPayerBalanceByAddress(payerId, tokenId, merchantId);
...
func main() {
	...
	resp, r, err := apiClient.PaymentAPI.GetPayerBalanceByAddress(ctx).
		PayerId(payerId).
		TokenId(tokenId).
	...
	if err != nil {
		fmt.Fprintf(
			os.Stderr,
			"Error when calling `PaymentAPI.GetPayerBalanceByAddress``: %v\n",
			err,
		)
...
apiInstance.getPayerBalanceByAddress(payer_id, token_id, opts).then(
  (data) => {
