...
with cobo_waas2.ApiClient(configuration) as api_client:
    ...
    try:
        ...
        print(
            "Exception when calling PaymentApi->get_destination_detail_by_id: %s\n" % e
        )
...
func main() {
	...
	if err != nil {
		fmt.Fprintf(
			os.Stderr,
			"Error when calling `PaymentAPI.GetDestinationDetailById``: %v\n",
			err,
		)
...
apiInstance.getDestinationDetailById(destination_id).then(
  (data) => {
