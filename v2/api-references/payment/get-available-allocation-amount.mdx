...
public class Example {
  public static void main(String[] args) {
    ...
    try {
      PaymentAllocationAmount result =
          apiInstance.getAvailableAllocationAmount(tokenId, sourceAccount, destinationAccount);
...
func main() {
	...
	resp, r, err := apiClient.PaymentAPI.GetAvailableAllocationAmount(ctx).
		TokenId(tokenId).
	...
	if err != nil {
		fmt.Fprintf(
			os.Stderr,
			"Error when calling `PaymentAPI.GetAvailableAllocationAmount``: %v\n",
			err,
		)
