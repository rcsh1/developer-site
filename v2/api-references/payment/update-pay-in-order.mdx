---
openapi: put /payments/orders/{order_id}
---
...
with cobo_waas2.ApiClient(configuration) as api_client:
    ...
    update_payment_order_request = cobo_waas2.UpdatePaymentOrderRequest(expired=True)

    try:
        # Update pay-in order
        api_response = api_instance.update_payment_order(
            order_id, update_payment_order_request=update_payment_order_request
        )
...
public class Example {
  public static void main(String[] args) {
    ...
    UpdatePaymentOrderRequest updatePaymentOrderRequest =
        new UpdatePaymentOrderRequest().expired(true);
    try {
...
func main() {
	...
	resp, r, err := apiClient.PaymentAPI.UpdatePaymentOrder(ctx, orderId).
		UpdatePaymentOrderRequest(updatePaymentOrderRequest).
		Execute()
	if err != nil {
...
const opts = {
  UpdatePaymentOrderRequest:
    CoboWaas2.UpdatePaymentOrderRequest.constructFromObject({ expired: true }),
};
apiInstance.updatePaymentOrder(order_id, opts).then(
  (data) => {
