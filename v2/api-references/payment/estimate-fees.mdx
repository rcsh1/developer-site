    ...
    payment_estimate_fee_request = cobo_waas2.PaymentEstimateFeeRequest(
        estimate_fees=[
            cobo_waas2.PaymentEstimateFee(
                token_id="ETH_USDT", amount="500.00"
            )
        ]
    )
  ...
  public static void main(String[] args) {
    ...
    PaymentEstimateFeeRequest paymentEstimateFeeRequest =
        new PaymentEstimateFeeRequest()
            .estimateFees(
                Arrays.asList(
                    new PaymentEstimateFee().tokenId("ETH_USDT").amount("500.00")));
    try {
...
func main() {
	paymentEstimateFeeRequest := *coboWaas2.NewPaymentEstimateFeeRequest([]coboWaas2.PaymentEstimateFee{*coboWaas2.NewPaymentEstimateFee("ETH_USDT", "500.00")})
...
const opts = {
  PaymentEstimateFeeRequest:
    CoboWaas2.PaymentEstimateFeeRequest.constructFromObject({
      estimate_fees: [{ token_id: "ETH_USDT", amount: "500.00" }],
    }),
};
