...
with cobo_waas2.ApiClient(configuration) as api_client:
    ...
    try:
        ...
        print(
            "Exception when calling PaymentApi->get_counterparty_detail_by_id: %s\n" % e
        )
...
func main() {
	...
	if err != nil {
		fmt.Fprintf(
			os.Stderr,
			"Error when calling `PaymentAPI.GetCounterpartyDetailById``: %v\n",
			err,
		)
...
apiInstance.getCounterpartyDetailById(counterparty_id).then(
  (data) => {
