...
You can call Payment API to get dedicated top-up addresses for payers. The diagram below shows the interaction flow between payers, merchants, developers, and Cobo:
...
sequenceDiagram
    ...
    participant Cobo
    Payer ->> Merchant: Request to enable wallet/top-up function
    Merchant ->> Developer: Request to enable wallet
    Developer ->> Cobo: ① Get top-up address for this payer
    ...
    Payer ->> Cobo: Transfer funds to address for top-up
    ...
    Merchant ->> Payer: Update wallet balance
```
...
1. Call [Get top-up address](/v2/api-references/payment/get-top-up-address) to create a top-up address for a specified payer of a specified merchant. Key parameters in the request include:
   ...
   * **Custom payer ID** (`custom_payer_id`): The unique identifier of the payer in your system.
...
2. **Configure top-up event listening**: Receive top-up related event notifications through webhooks, including:
   ...
   - `payment.transaction.held`: Deposit transaction is being screened for compliance
...
- You can refer to [Settlement](/v2/payments/settle-topup) to settle merchant balance or developer balance in top-up mode.
- To handle situations such as address changes and compliance abnormalities, please refer to [Handle abnormality](/v2/payments/handle-abnormality-topup).
