...
Call the "Create refund link" API:
...
The following is sample code for calling the "Create refund link" API:
...
with cobo_waas2.ApiClient(configuration) as api_client:
   ...
   try:
       ...
       print("Refund address collection URL:", api_response.url)
       print("Refund request identifier:", api_response.token)
...
public class Example {
 public static void main(String[] args) {
   ...
   createRefundLinkRequest.setTransactionId("<ORIGINAL_TRANSACTION_ID>");
   createRefundLinkRequest.setAmount("100");
   createRefundLinkRequest.setRefundSource(RefundSource.MERCHANT);
   createRefundLinkRequest.setMerchantId("<YOUR_MERCHANT_ID>");
   createRefundLinkRequest.setFeeAmount("0");

   try {
          RefundLink refundLink = apiInstance.createRefundLink(createRefundLinkRequest);
          System.out.println("Refund address collection URL: " + refundLink.getUrl());
          System.out.println("Refund request identifier: " + refundLink.getToken());
...
<img src="/v2/images/payments/refund-address-collection-default.png" className="screenshot_full_screen" alt="Refund address collection page - default address" />

**Change refund address page** (requires filling in new address and Memo):

<img src="/v2/images/payments/refund-address-collection-change.png" className="screenshot_full_screen" alt="Refund address collection page - change address" />
...
```py
try:
   refund_detail = api_instance.get_refund_detail(refund_id="<REFUND_ID>")
   print("Refund status:", refund_detail.status)
   print("Refund amount:", refund_detail.amount)
   print("Refund address:", refund_detail.to_address)
except Exception as e:
   print("Exception when calling PaymentApi->get_refund_detail: %s\n" % e)
```
</Accordion>
...
|------------|-------------|---------------------|
| `Insufficient_balance` | Insufficient refundable amount | Check account balance to ensure there are sufficient funds for the refund |
| `refund_in_progress` | Refund already in progress | Do not manually refund orders that have already entered the automatic refund process in the backend to avoid duplicate payouts |
