...
| `payment.transaction.held` | Deposit transaction failed compliance screening. In order mode, this event is triggered even if order has reached final status (`Underpaid`, `Expired` or `Completed`). | [PaymentTransaction](/v2/payments/status-and-events#paymenttransaction) |
