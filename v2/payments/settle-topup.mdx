...
Call [Get withdrawable balances](/v2/api-references/payment/get-withdrawable-balances) to query specified merchant balance or developer balance and determine the maximum amount you can settle. In top-up mode, you need to set the `acquiring_type` field to `TopUp`.

The API returns two key balance fields:
...
- Call [Create forced sweep](/v2/api-references/payment/create-forced-sweep) to perform forced sweep: Cobo will immediately sweep funds that have not reached the threshold to the sweep-to address. After sweeping is complete, you can use these funds for settlement.
