...
- `apikey`: Use the API Key to authenticate.
- `user`: Log in with your Cobo Portal account.
- `org`: Use the Org Access Token to authenticate.
...
To use this method, you need to generate an API key pair and register the public key in [Cobo Portal](https://portal.cobo.com).

1. Generate an API key pair by running the following command.
    ```shell
    cobo keys generate
    ```
