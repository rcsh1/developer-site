...
- Page identifier: Identifying the page or module's feature, such as ` home`, `login`, or `profile`.
- Component identifier: Identifies the specific UI component within the page, such as `form`, `navbar`, or `modal`.
- Element identifier: Identifies the button or interactive element, such as `submit`, `register`, or `next_step`.
