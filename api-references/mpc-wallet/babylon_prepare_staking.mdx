...
description: "This endpoint is used to create a pre-signed staking transaction, accepting parameters such as `request_id`, `stake_info`, `fee_rate`, and `max_staking_fee`. Please note that a successfully created staking transaction will not be broadcast automatically; you need to call the [Babylon Broadcast Staking Transaction](/api-references/mpc-wallet/babylon_broadcast_staking_transaction) endpoint to broadcast it."
---
