...
- Destination: For a transaction, the source refers to the party that receives the transaction. It can be a Custodial Wallet, an MPC Wallet, a Smart Contract Wallet, an Exchange Wallet, an external address, a message to be signed, or a smart contract. In a token transfer, the source refers to the deposit address.
