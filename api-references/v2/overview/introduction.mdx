...
//TODO 是否需要列出不同环境的区别，比如 dev 才能用 playground
...
| Field | Description | 
| ----------- | ----------- |
| `Biz-Api-KeyAPI` | An API key. To learn how to create an API key, see [ API key](). //TODO 链接到生成 api key 的 guide？   | 
...
1. Concatenate the string to be signed
    ...
    | PATH  | The request path.   | `/v2/transactions/transfer`//TODO 这个路径需要包含host吗 |
...
| `2020`| Missing action parameter. //TODO 不懂 action 指什么 |
| `2021`| Missing handler definition. //TODO 是说没设置 webhook 吗|
| `2022`| Missing required request headers. |
| `2023`| Verification failed. //TODO 哪方面的验证？|
| `2024`| Authentication failed. //TODO api 的鉴权吗|
| `2025`| Forbidden access to resources. //TODO api key 的权限吗|
| `2026`| Too many requests. //TODO 有限制吗|
| `2027`| Quota limit exceeded. //TODO 同上|
| `2028`| Resource not found. //TODO 就是 404 吗|
| `2029`| Invalid status. //TODO 什么的状态？|
| `2040`| Duplicate key violation. //TODO 什么情况会出现这个错误|
...
## Webhook events 
Upon receiving a deposit or withdrawal request, the WaaS service sends a push message that contains the transaction data in JSON format as an HTTP POST request to a user-defined URL. To learn more about how to subscribe to webhook events, see [Webhook events]().//TODO 链接 tbd

## Callback messages
When you receives a webhook event and want to approve or reject the request, you need to respond promptly with an HTTP body containing either `ok` for approval or `deny` for rejection with an HTTP status of `200`. Failure to provide this response will result in delayed request processing and continued pushes of the same message to the designated URL. To learn more about how to subscribe to callback messages, see [Callback messages]().//TODO 链接 tbd

## Glossary

- Asset: (This concept only applies to exchange wallets) A digital representation of a valuable resource on a blockchain network. Exchange wallets group your holdings by asset, even if the same asset exists on different blockchains, for example, Bitcoin on Ethereum and Bitcoin on its native chain.
...
- API key: A unique identifier used to authenticate and authorize API requests. An API key allows an organization to initiate specific actions such as withdrawals. To learn how to create an API key, see [API keys]().//TODO 链接
...
- Address: A unique identifier on a blockchain network used to send and receive cryptocurrency or other digital assets. It is recommended that you use a dedicated address for transactions of one token for MPC Wallets.//TODO 需要确认
...
- Transaction fee: A fee paid to miners or validators on a blockchain network to process and confirm a transaction. Transaction fee varies based on the fee model that the chain uses.
